...
- Dependencies:
  ...
  - `lz-string` is served locally from `dist/js/lz-string.min.js` (populated by `/src` build step).
...
- Keep or update accessible labels/roles to match tests. If you change visible text or structure, update tests alongside the UI changes.
- Typical test commands from `src/`:
...
- Do:
  ...
  - Run `/src` build before committing changes that affect Bootstrap styling.
- Don’t:
...
- Update tests in `/src/tests` if UI text/structure or behaviors changed.
- Verify local run, import/export URLs, and all operating modes (Standard/AWS/Azure/OCI).

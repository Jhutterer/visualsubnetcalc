(function () {
  ...
  function ensureSqliteModule() {
    ...
    if (typeof sqlite3InitModule !== 'function') {
      ...
      throw new Error('SQLite wasm module not loaded: sqlite3InitModule is undefined');
  ...
  class PlannerDbManager {
    ...
    async openWithFilePicker() {
      ...
      await this.openDatabaseFromUint8Array(new Uint8Array(await file.arrayBuffer()), handle.name || file.name || 'Planner database');
    ...
    runMigrations() {
      ...
      if (currentVersion > TARGET_VERSION) {
        ...
        console.warn('Database user_version is newer than supported version', currentVersion);
    ...
    async insertSampleBuilding() {
      ...
      const existing = this.selectAll('SELECT id FROM tbl_building WHERE slug = ?', ['sample-hq']);
    ...
    async savePlannerSnapshot(snapshot) {
      ...
      const baseNetwork = typeof snapshotData.baseNetwork === 'string' ? snapshotData.baseNetwork : '';

      const operatingModeInput = typeof snapshotData.operatingMode === 'string' ? snapshotData.operatingMode.trim() : '';
      ...
      await this.runInTransaction(async () => {
        ...
        const insertNode = (node, parentId, fallbackOrdinal) => {
          ...
          const ordinal = Number.isFinite(preferredOrdinal) ? preferredOrdinal : (Number.isFinite(fallbackOrdinal) ? fallbackOrdinal : 0);

          this.db.exec({

            sql: 'INSERT INTO planner_subnet (parent_id, cidr, note, color, ordinal) VALUES (?, ?, ?, ?, ?);',

            bind: [parentId ?? null, node.cidr, note, color, ordinal],
          ...
          if (Array.isArray(node.children)) {
            ...
            node.children.forEach((child, index) => insertNode(child, insertedId, index));
    ...
    async loadPlannerSnapshot() {
      ...
      const [stateRow] = this.selectAll('SELECT id, base_network AS baseNetwork, operating_mode AS operatingMode, updated_at AS updatedAt FROM planner_state WHERE id = 1 LIMIT 1;');

      const rows = this.selectAll(

        'SELECT id, parent_id AS parentId, cidr, note, color, ordinal FROM planner_subnet ORDER BY id ASC;',
    ...
    getDiagnostics() {
      ...
      return {
        ...
        buildingCount: hasDb ? this.selectAll('SELECT COUNT(*) as total FROM tbl_building;')[0]?.total || 0 : 0,
  ...
  ready(async () => {
    ...
    const buildingTableBody = document.querySelector('#db-buildings-table tbody');
    ...
    const render = () => {
      ...
      if (diag.hasDatabase) {
        ...
        const label = diag.mode === 'memory'

          ? 'in-memory'

          : (diag.fileHandleName ? `file: ${diag.fileHandleName}` : diag.sourceLabel);
    ...
    const showFeedback = (message, type = 'info') => {
      ...
      feedbackEl.classList.add('small', type === 'error' ? 'text-danger' : 'text-muted');
    ...
    try {
      ...
      showFeedback('Failed to load SQLite engine. Check console for details.', 'error');
      ...
      try {
        ...
        if (manager.supportsFileSystemAccess()) {
          ...
          try {
            ...
            showFeedback('Created planner database file. Changes will auto-save locally.');

            return;

          } catch (err) {

            console.warn('PlannerDb File System Access create failed, falling back', err);
      ...
      saveBtn.addEventListener('click', async () => {
        ...
        try {
          ...
          if (!manager.fileHandle) {
            ...
            showFeedback('Open or create a planner database file before saving.', 'error');
          ...
          const fileName = manager.fileHandle?.name ?? manager.sourceLabel ?? 'planner.sqlite';
    ...
    openBtn.addEventListener('click', async () => {
      ...
      if (manager.supportsFileSystemAccess()) {
        ...
        try {
          ...
          showFeedback(`Loaded planner database from ${manager.fileHandle?.name ?? manager.sourceLabel}.`);
          ...
          showFeedback('Unable to open planner database via File System Access.', 'error');
